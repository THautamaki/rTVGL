...
The package is a work in progress, and all documentation will be added in the future.
...
Generate simulated data using the `generate_timeseries_network_data` function. We use the R package `MASS` to generate multivariate normally distributed samples, which uses the eigen decomposition in its generation mechanism. We noticed that different basic linear algebra subprograms (BLAS), such as OpenBLAS, handle the signs of eigenvectors differently. Thus, the generated samples come from the same distribution, but the values differ even when the same seeds are used.
